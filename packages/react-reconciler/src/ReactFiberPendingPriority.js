...
// 将 expirationTime 记录到 fiber 的 earliestPendingTime 或者 latestPendingTime
//  在 earliestPendingTime 和 latestPendingTime 区间外的 新的 expirationTime 会作为新的边界值
...
// expirationTime 在哪里比较?
function findNextExpirationTimeToWorkOn(completedExpirationTime, root) {
  const earliestSuspendedTime = root.earliestSuspendedTime;
  const latestSuspendedTime = root.latestSuspendedTime;
  const earliestPendingTime = root.earliestPendingTime;
  // lastestPingedTime commit phrase 时会设置的值
  const latestPingedTime = root.latestPingedTime;
