...
function resetStack() {
  if (nextUnitOfWork !== null) {
    // 如果此时有安排中的下一个更新的 fiber, 记录其根 fiber
    // ? how ? when ? where
...
function renderRoot(
  ...
  // 这个断言用于检查是否递归调用了本函数.
  // Fiber 的整体调用由 scheduler 控制. renderRoot 中处理的 root 的这个过程不是线性的
  // 而是将处理过程一片一片的做切片处理, 同时只处理 一个 root Fiber
  invariant(
    !isWorking,
  ...
  if (
    // 检查当前的和之前的一个 fiber 环境中的 expirationTime , 不同则说明这是一次全新的更新
    /**
     * 如果当前是一段用户操作引发的, 那么 expirationTime 必不等于 nextRenderExpirationTime
     */
    expirationTime !== nextRenderExpirationTime ||
    // root 不相同 , 全新更新
    root !== nextRoot ||
    nextUnitOfWork === null // 下一个任务为空, 全新更新
  ) {
...
function computeExpirationForFiber(currentTime: ExpirationTime, fiber: Fiber) {
  ...
  if (expirationContext !== NoWork) {
    ...
    // isWorking 表示当前的这个 update 是发生在 renderFiberRoot 的过程中(render phrase) 的
    if (isCommitting) {
      ...
      // 不再 commit phrase 之中, 这个 update 和当前的 working fiber 会在同一个时刻被处理 (rendered)
      expirationTime = nextRenderExpirationTime;
    }
  } else {
    // No explicit expiration context was set, and we're not currently
    // performing work. Calculate a new expiration time.
    // 当前是"静止", 这个 update 是第一个
    // 如果没有开启 AsyncMode (开启的方式是使用 Async Node), 目前这里一直会得到 false
...
/** 将当前的 fiber 记入更新计划
 *  会把当前 fiber 的 child 的 expirationTime 更新为 传入的参数
 *
 **/

// 返回的可能为 null, 意味着?
// 如果有返回值, 返回的是 HostRoot 的 "stateNode" . 这个 stateNode 指的是 当前的 fiber 的 instance (new class ....)
// Root 说的是这一条的 根 fiber
function scheduleWorkToRoot(fiber: Fiber, expirationTime): FiberRoot | null {
  // Update the source fiber's expiration time

  //  fiber.expirationTIme > expiration 出现的情况 有 interactiveFiber 下一个更新是 Sync
  //  如果相反则是由 NoWork -> sync, 或 -> 大于 Sync
  if (
...
function scheduleWork(fiber: Fiber, expirationTime: ExpirationTime) {
  ...
  /**
    *  --- 敲黑板
    *      root 是当前触发更新的 fiber 的 "根" (通过 fiber.return 找到的) 的 stateNode
    *         这个 根 是指: 如果是同一个 根 Component, 里面的子孙组件的这个 root 总是一样的
    *
    *      它的 current 是真正进行更新的 fiber 链条 ⛓
    *      这个 current 指向的永远是上一次更新创建的 alternate, 在这一次更新的时候以 上一次的 alternate
    *      创建一个新的 alternate
    *      所以实际上在 React 中可以同时存在 三个 fiber 来代表当前的 class
    *      一个是插入时 new 出时的创建的  nFiber (只参与插入和第一次更新, 后续只是一个入口, 用来获取 stateNode)
    *      一个是第一次更新时 创建的 uFiber
    *      当第二次更新的时候 会以 uFiber 为蓝本创建一个 sFiber (bailoutOnAlreadyFinishedWork 的 cloneFibers 中)
    *      sFiber 会在第二次更新完成的时候取代(在 commitRoot 中)uFiber 的位置参与后续的更新
    *      共同点是他们都会指向 同一个 stateNode 也就是当前 class 的 instance
    */
  ...
  if (
    /**
     *
     * 除非当前正在 render phase 的 fiber 不是触发更新的 fiber
     * 否则不需要 schedule fiber 的更新, 因为 更新计划已经添加进了 updateQueue
     *
     * 如果当前已经是 commit phase (这时相同 fiber 的更新 已经不能合并进去了), 所以需要计划更新
     *
     */
...
function requestWork(root: FiberRoot, expirationTime: ExpirationTime) {
  ...
  /**
   *
   * 此时除了进行已经正在进行的 fiber root 会进行合并操作外,
   * 其余的操作会进入链表(lastScheduleRoot) 等待
   *
   */
  if (isRendering) {
  ...
  /**
   * isBatchingUpdates  为true 时 表示目前有 用户在进行交互操作
   * isUnbatchingUpdates  表示这一段用户的操作已经完成,
   * 而这段时间内的更新要马上反馈给用户, 这里直接调用 performWorkOnRoot
   *
   *
   */
...
// 添加当前的有更新操作的 fiber 到更新链表中
// 这个链表由 当前 module 维持, head: firstScheduledRoot   tail: lastScheduledRoot
function addRootToSchedule(root: FiberRoot, expirationTime: ExpirationTime) {
  ...
  if (root.nextScheduledRoot === null) {
    ...
    if (lastScheduledRoot === null) {
      ...
      lastScheduledRoot.nextScheduledRoot = root; // 这行没用吧?
      lastScheduledRoot = root;
      lastScheduledRoot.nextScheduledRoot = firstScheduledRoot;
    }
  } else {
    // This root is already scheduled, but its priority may have increased.
    // 从这一段可以知道, update 的优先级 (priority) 是用 expiration 来计算的
    // 并且 expiration 的值越小, 优先级就越高 (所以 Sync 是优先级最高的, NoWork 为 0, 则是最低)
    ...
    while (root !== null) {
      ...
      if (remainingExpirationTime === NoWork) {
        ...
        if (highestPriorityWork === Sync) {
          // 因为 Sync 的优先级最高, expirationTime  越低优先级越高
...
function performWork(minExpirationTime: ExpirationTime, dl: Deadline | null) {
  ...
  if (deadline !== null) {
    ...
    while (
      // 下一个要渲染的也是优先级最高的 root fiber
      nextFlushedRoot !== null &&
      // 这个 root fiber 的 expirationTime 不能为 NoWork
      nextFlushedExpirationTime !== NoWork &&
      // 当前phrase 的 expirationTime 为最低优先
      // 如果其为 NoWork 或者 不小于 当前更新的 root 的 expirationTime
      (minExpirationTime === NoWork ||
        /**
         * 这个判断也非常重要:
         *     由于这是在 while 循环中, 假设当前是 triangle demo,
         *     那么 nextFlushedRoot 和 nextFlushedExpirationTIme 的两者的比较 不可能会 false
         *     并且大部分也都是 Sync 的优先级更新, 按逻辑来说会一直在更新, 但是此时要响应用户交互的更新
         */
        minExpirationTime >= nextFlushedExpirationTime)
    ) {
      // 执行 root fiber 的 work
      performWorkOnRoot(nextFlushedRoot, nextFlushedExpirationTime, true);
      // 执行完毕后 查找下一个有 work 的 root.
      // 这个是由整个 module 来维持这样的一个单例的. 找到的会赋值给 nextFlushedRoot
      findHighestPriorityRoot();
    }
  }
...
function performWorkOnRoot(
  ...
  if (deadline === null || isExpired) {
    ...
    /**
     * finishedWork 记录当前 root fiber 是否完成 renderRoot 的工作
     * 为什么要进行二次检测?
     *  我猜是因为与 fiber 会中断当前的任务有关. 避免重复的 renderRoot .
     */
    ...
    if (finishedWork !== null) {
      ...
      renderRoot(root, isYieldy, isExpired); // = renderRoot(root, false, true);
